...
from backend.forms import  ProfileUpdateForm, ProfileAddForm, TransactionAddForm, CreateAdminUserForm, TransactionUpdateForm, ContactUserForm, EditAdminUserForm, transactionPhonenumberSearchForm, ChargesLimitsForm, RateUpdateForm
import remitapi.settings as settings
#from remitapi.utils import generate_sha1, mailer, sendsms, error_message, success_message
from remitapi.utils import error_message, success_message, admin_mail, sendsms, mailer
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
from remitapi.models import ApiTransaction, Country, Charge
from accounts.models import UserProfile, AdminProfile, UserActions
from accounts.forms import AdminLoginForm
from backend.decorators import admin_required, superuser_required, permission_required, customer_care_required
from django.db.models import Q
from datetime import datetime,  timedelta
from django.db.models import Sum, Max
from django.contrib import messages
from django.db import IntegrityError
import backend.utils as admin_utils
import urllib2
from django.core.files.base import ContentFile
from StringIO import StringIO
from PIL import Image
from remitapi.utils import debug , log_unauthorized_access, render_to_pdf
...
from backend.utils import log_action, store_login_info
...
def dashboard_stats(request):
    ...
    if request.user.is_active and request.user.is_staff:
        ...
        profile = User.objects.filter(is_superuser=False, is_staff=False).count()
        ...
        #transaction = ApiTransaction.objects.filter(
        ...
        for country in countries:
            ...
            data['amount_pending_%s' % currency] = transaction['amount_received__sum']
        ...
        data['user_with_transaction'] = ApiTransaction.objects.filter(
            visa_success=True, is_processed=True, amount_sent__isnull=False).values('user').distinct().count()
        data['complete_transactions'] = ApiTransaction.objects.filter(
...
def render_view(request, template, data):
    ...
    data['STATIC_URL'] = '%sbackend/' % settings.STATIC_URL
    template = 'backend/%s' % template
    print template
...
def home(request):
    if request.user.is_superuser:
        ...
        return render_view(request, 'index_staff.html', {})
...
def unblock_user(request):
    ...
    #return HttpResponseRedirect(settings.BASE_URL + 'users/verified/')
...
def block_user(request):
    ...
    if request.POST:
        if not 'block_user' in request.POST:
            ...
            try:
                profile.save()
                log_action(request, model_object=profile, action_flag=15, change_message='blocked user')
                success_message(
...
def verify_user(request):
    ...
    if request.POST:
        if not 'verifyuser' in request.POST:
            ...
            try:
                profile.save()
                log_action(request, model_object=profile, action_flag=9, change_message='verified user')
                success_message(
...
def view_transaction(request,name):
    ...
    transaction = get_object_or_404(ApiTransaction.objects.filter(pk=name))
    log_action(request,model_object=transaction, action_flag=6, change_message='Viewed Transaction')
    return render_view(request, 'ApiTransaction.html', 
...
def edit_transaction(request,name):
    ...
    if request.POST:
        ...
        if form.is_valid():
            form.save()
            ApiTransaction.updated_by = request.user
            ApiTransaction.save()
            success_message(request, 'admin_edit_transaction', {})
            admin_mail(request, 'transaction_updated', {'transaction': transaction})
            log_action(request,model_object=transaction, action_flag=9, change_message='edited Transaction')
    return render_view(request, 'edit_ApiTransaction.html', 
        {'transaction': transaction,'form':form})


def stuff_transaction_list(user,status=1):
    '''status (1)-successful,(2)-pending,(3)-Failed,(4)-All'''
    transaction_list = False
    if status == 1:
        transaction_list = ApiTransaction.objects.filter(
            visa_success=True, is_processed=True, amount_sent__isnull=False)
    elif status == 2:
        transaction_list = ApiTransaction.objects.filter(visa_success=True, is_processed=False, amount_sent__isnull=False)
    elif status == 3:
        transaction_list = ApiTransaction.objects.filter(visa_success=False)
    elif status == 4:
        transaction_list = ApiTransaction.objects.all()
    ...
    if transaction_list and not user.is_superuser:
        ...
        #transaction_list = ApiTransaction.objects.filter(country_filter & network_filter)
        transaction_list = transaction_list.filter(country_filter & network_filter)
...
def transactions(request, name=False, user_id=False):
    ...
    log_action(request,model_object=transaction_list, action_flag=6, change_message='view Transaction')
    return render_view(request, 'transactions.html', {'transactions': transactions, 'pretitle': pretitle, 'page_title': page_title, 'type': name})
...
def resend_transaction(request):
    ...
    if request.POST:
        if not 'resend_transaction' in request.POST:
            ...
            id = name ^ 0xABCDEFAB
            transaction = get_object_or_404(ApiTransaction.objects.filter(
            ...
            transaction = get_object_or_404(ApiTransaction.objects.filter(id=id))
            if ApiTransaction.is_processed:
                success_message(request, 'admin_resend_transaction', {'response':response})
            #else:
            #        error_message(request, 'admin_resend_transaction', {'response': response})
            else:
                error_message(request, 'admin_process_transaction', {'response': response})
    else:
        return HttpResponseRedirect(reverse('custom_404'))
...
def process_transaction(request):
    ...
    '''
    if request.POST:
        if not 'process_transaction' in request.POST:
            return HttpResponseRedirect(reverse('custom_404'))
        else:
            name = request.POST['process_transaction']
    ...
    #return HttpResponseRedirect(settings.BASE_URL + 'transactions/successful/')
    return HttpResponseRedirect(reverse('admin:admin_user_transactions', args=['pending']))
...
def users(request, name):
    ...
    log_action(request, model_object=user_list, action_flag=13, change_message='searched user')
    return render_view(request, 'users.html', {'users': users, 'pretitle': pretitle, 'page_title': page_title})
...
def stuff_users(request, name=False):
    ...
    try:
        ...
        # If page is out of range (e.g. 9999), deliver last page of results.
        users = paginator.page(paginator.num_pages)
    return render_view(request, 'stuff_users.html', {'users': users})
...
def user(request, name):
    ...
    if not check_user_permission(request.user, 'edit_profile'):
        # print "NOOOOO"
        return render_view(request, 'user_readonly.html', {'name': name,
        ...
        transaction_stats = ApiTransaction.objects.filter(
        ...
        if userdata['successful_transactions'] > 0:
            ...
            if 'amount_sent__max' in top_amount_sent:
                userdata['top_amount_sent'] = top_amount_sent[
                    'amount_sent__max']
        return render_view(request, 'user.html', {'name': name,
                                                        'user_profile': profile,
                                                        'pretitle': pretitle,
                                                        'page_title': page_title,
                                                      'userdata': userdata})
...
def charges_limits(request, code):
    ...
    if request.POST:
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "The Charges & Limits Was Successfully Edited")
        else:
            print form.errors
    return render_view(request, 'charges_limits.html', 
...
def rates(request, code):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            print form.errors  
    return render_view(request, 'rates.html', 
...
@admin_required
def logs(request):
    return render_view(request, 'logs.html', {})
...
def save_transaction(cur,user,pending = False):
    for row in cur.fetchall():
        ...
        if datarow:
            ...
            if ApiTransaction.is_valid():
                try:
                    ApiTransaction.save()
                except IntegrityError as e:
                    print e
            else:
                print ApiTransaction.errors
...
def audits_trails(request):
    ...
    if 'start_date' in request.GET:
        ...
        first_log_entry = LogEntry.objects.values_list('action_time', flat=True).order_by('id')[:1]
    ...
    if start_date == end_date:
        audit_logs_list =  audit_logs_list.filter(log_entry__action_time__contains=start_date.date())
    else:
        audit_logs_list =  audit_logs_list.filter(log_entry__action_time__range=(start_date, end_date))
...
def reports(request):
    ...
    if 'start_date' in request.GET:
        ...
        first_successful_transaction = ApiTransaction.objects.filter(
            visa_success=True, 
            is_processed=True, 
            amount_sent__isnull=False
            ).values_list(
        'processed_on', flat=True).order_by('id')[:1]
        #start_date = "%d-%m-%Y".format(rstart_date[0]) #datetime.strptime(rstart_date[0], '%d-%m-%Y')
    ...
    countries_list = transaction_list.values_list('sender_country', flat=True).distinct()

    if start_date == end_date:
        transaction_list = transaction_list.filter(
            Q(started_on__startswith=start_date.date()) | Q(started_on__startswith=end_date.date()),
            )
        #print transaction_list
    else:
        transaction_list = transaction_list.filter(
            Q(started_on__range=(start_date, end_date)) | Q(started_on__startswith=start_date.date()) | Q(started_on__startswith=end_date.date())
            )
    ...
    if transaction_list:
        ...
        if transaction_list:
            ...
            if settings.IS_SQLITE: 
                ...
                #    'user').count()
                number_of_unique_senders = transaction_list.values_list('user', flat=True).distinct()
...
def edit_stuff_user(request, name):
    ...
    return render_view(request, 'edit_stuff_user.html', 
    ...
    if user:
        ...
        #customer care options
            ...
            if update and form.cleaned_data['transactions'] == '1':
                user.user_permissions.remove(edit_transactions, view_transaction)
...
def contact_user(request, name):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            mailer(request, data['subject'],template, data, profile.user.email)
            messages.success(request,'The Message Was Successfully sent')
    support_emails = EmailSupport.objects.filter(user=profile.user).order_by('-id')
...
def generate_csv_report(transaction, user=False, _file=False):
    ...
    if _file:
        ...
        response['Content-Disposition'] = 'attachment; filename="report_%s.csv"' % date
    ...
    if request.GET:
        ...
        if form.is_valid():
            ...
            try:
                ...
                transaction_list = ApiTransaction.objects.filter(pk=invoice_id)
            except Exception, e:
                print e
                pass
...
def export_data(request):
    ...
    if request.POST:
        ...
        response['Content-Disposition'] = 'attachment; filename="export_%s_%s.csv"' %(filename,date)
        import csv
        from django.utils.encoding import smart_str
