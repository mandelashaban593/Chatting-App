...
def users_pending_verification(count=False):
    ...
    user_list = UserProfile.objects.filter(
        ~Q(id_pic=''), account_verified=False, id_verified=False, account_blocked=False)
    if count:
...
def unverified_users(count=False):
    '''get pending users'''
    user_list = UserProfile.objects.filter(Q(id_pic=''),account_verified=False, id_verified=False, account_blocked=False,user__isnull=False)
    if count:
        user_list = user_list.count()
    return user_list
